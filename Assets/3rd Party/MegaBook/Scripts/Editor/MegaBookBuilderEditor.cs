...
public class MegaBookBuilderEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		mod.dontBuildOnPlay = EditorGUILayout.Toggle("Dont Build On Play", mod.dontBuildOnPlay);
		...
		mod.UseThreading = EditorGUILayout.Toggle("Multi Thread", mod.UseThreading);
		...
		mod.shaderTexture = EditorGUILayout.TextField("Shader Texture Name", mod.shaderTexture);
		...
		if ( mod.showmeshoptions )
		{
			...
			bval = EditorGUILayout.Toggle("Use UV2", mod.useuv2);
			if ( bval != mod.useuv2 )
			{
				mod.useuv2 = bval;
				updatemesh = true;
			...
			Transform anc = (Transform)EditorGUILayout.ObjectField("Anchor Override", mod.anchorOverride, typeof(Transform), true);
			if ( anc != mod.anchorOverride )
			{
				mod.anchorOverride = anc;
				updatemesh = true;
			}
			...
			LightProbeUsage lp = (LightProbeUsage)EditorGUILayout.EnumPopup("Light Probe Usage", mod.lightProbeUsage);
			if ( lp != mod.lightProbeUsage )
			{
			...
			MotionVectorGenerationMode mv = (MotionVectorGenerationMode)EditorGUILayout.EnumPopup("Motion Vectors", mod.motionVectors);
			if ( mv != mod.motionVectors )
			{
				mod.motionVectors = mv;
				updatemesh = true;
			}
#endif
#endif
		...
		{
			mod.pages[i].turnerfromcon = EditorGUILayout.CurveField("Turn " + i, mod.pages[i].turnerfromcon);
		}
