...
#if UNITY_5_4 || UNITY_5_5 || UNITY_5_6 || UNITY_2017 || UNITY_2018 || UNITY_2019 || UNITY_2020 || UNITY_2021
...
#if false
// Example class for using the MegaBook Events
public class BookListen : MonoBehaviour
{
	...
	void Start()
	{
		if ( book )
		{
			book.pageTurnEvent.AddListener(TestTurn);
			book.bookBuiltEvent.AddListener(TestBuilt);
		}
	}

	void TestBuilt(MegaBookBuilder book)
	{
		Debug.Log("Book Built");
	}

	void TestTurn(MegaBookBuilder book, float delta)
	{
		Debug.Log("Page Turned " + delta);
	}
}
#endif
...
public class MegaBookBuilder : MonoBehaviour
{
...
#if UNITY_5_4_OR_NEWER || UNITY_2017 || UNITY_2018 || UNITY_2019 || UNITY_2020 || UNITY_2021
...
#endif
		//pageTurnEvent.AddListener(TestTurn);
		//bookBuiltEvent.AddListener(TestBuilt);
	...
	GameObject MakePageObject(MegaBookPage page, int pnum)	//, Material mat)
	{
...
#endif
		cmr.lightProbeProxyVolumeOverride = proxyVolume;
#else
		...
		if ( pageobject || pp.pageobj )
		{
			...
			{
				if ( pp.madefront )
					mats[fm].SetTexture(shaderTexture, pp.madefront);
				else
					mats[fm].SetTexture(shaderTexture, pp.front);
			}
			...
			{
				if ( pp.madeback )
					mats[bm].SetTexture(shaderTexture, pp.madeback);
				else
					mats[bm].SetTexture(shaderTexture, pp.back);
			}
	...
	public void RebuildPageMesh(int num)
	{
		if ( num >= 0 && num < pages.Count )
			RebuildPageMesh(pages[num]);
	}

	public void RebuildPageMesh(MegaBookPage page)
	{
		MeshFilter mf = page.obj.GetComponent<MeshFilter>();
		MeshRenderer cmr = page.obj.GetComponent<MeshRenderer>();
		...
		if ( updatecollider )
		{
			page.collider = page.obj.GetComponent<MeshCollider>();
		}
		...
		if ( page.pnum > 0 && page.pnum < NumPages - 1 )
		{
			width += Random.Range(-1.0f, 1.0f) * pagesizevariation.x;
			length += Random.Range(-1.0f, 1.0f) * pagesizevariation.y;
			height += Random.Range(-1.0f, 1.0f) * pagesizevariation.z;
	...
	Mesh CreatePageMesh(MegaBookPage page, float width, float length, float height, int widthsegs, int lengthsegs, int heightsegs)
	{
		...
		if ( dynamobj && dynammeshenabled )
		{
			...
			if ( useuv2 )
			{
				textuv2s = dynamobj.GetUV2s(page.pnum, true);
			...
			if ( useuv2 )
			{
				textuv2s = dynamobj.GetUV2s(page.pnum, false);
				for ( int i = 0; i < textuv2s.Length; i++ )
					uv2s.Add(textuv2s[i]);
			}
